...
long long zRexp;
Znum zR1;    // = R mod N
Znum zR_1;   // = zR-1, used as bitmask
...
// NB. if N < 8 limbs  calculate NI2 as -N^(-1) mod 2^31
...
void GetMontgomeryParms(const Znum &Nval) {
	...
	assert((Nval & 1) != 0);		// N must be odd

	auto index = mpz_sizeinbase(ZT(Nval), 2);
	if (index > 1) {
		// (2^Rexp = R).
		zRexp = (index + BITS_PER_GROUP - 1) / BITS_PER_GROUP;	// get number of limbs 
		zRexp *= BITS_PER_GROUP;   // exp is rounded up to a multiple of 31
					       // to match what is used with limbs
		//zRexp--;
		zR = 1;
		mpz_mul_2exp(ZT(zR), ZT(zR), zRexp);   // zR is a power of 2 such that zR > Nval
		...
		mpz_powm_ui(ZT(zR2), ZT(zR), 2, ZT(Nval)); // zR2 = zR^2(mod Nval)
#ifdef _DEBUG
		std::cout << "N     = " << Nval << " (compare to TestNbr)"
			    << "\nNI    = " << zNI
...
Uses external global variables zR_1, zNI, zN*/
