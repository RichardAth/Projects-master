...
static void doTests3(void) {
	...
	LimbstoZ(aL, a1, numLen);       // copy value to a1 (Znum)
	...
	int length = BigNbrLen(alM, numLen);   // get real number length
	int length2 = BigNbrLen(al2, numLen);
	//length2 = std::max(length, length2);
	if (pm < 0) {
		ChSignBigNbr((int *)alM, numLen);  // make alM +ve
		MultBigNbr((int *)al2, (int *)alM, (int *)pl, length + length2);
	...
	for (int i = 1; i < 4560; i++) {
		...
		if (pBI.nbrLimbs > 2144)
			break;				   // stop loop here to make good comparision with 64 bit version
		if (!ZtoBig(amBI, p2))
			break;
	...
	p = 1;
	mpz_mul_2exp(ZT(p), ZT(p), 3 * 62 - 1);    // p = 2^(3*62)
	l2 = ZtoLimbs(pl, p, numLen);                // pl = p
	auto l3 = ZtoLimbs(bL, b, numLen);
	while (l3 < l2) {
		b *= 4;
		l3 = ZtoLimbs(bL, b, numLen);	            // copy value of b to bL (limbs)
	}
	AdjustModN(pl, bL, l3);             // pl %= b
	LimbstoZ(pl, pm, l3);               // pm = pl = p%b
	std::cout << "l2 = " << l2
		<< "\np   = " << p
		<< "\nb   = " << b
		<< "\np%b = " << p % b
	...
	elapsed = (double)end - start;
	std::cout << "test stage 5 completed  time used= " << elapsed / CLOCKS_PER_SEC << " seconds\n";
