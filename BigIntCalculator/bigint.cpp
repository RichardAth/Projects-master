...
BigInteger BigIntMultiply(const BigInteger &Factor1, const BigInteger &Factor2)
{
	...
	if (Factor1.nbrLimbs + Factor2.nbrLimbs > 67000 / BITS_PER_GROUP + 1)  // 2^66438 ~ 10^20000
	{
...
double logBigNbr(const Znum &BigInt) {
	double BigId;
	long BiExp;  // changed from long long to long for MPIR 3.0.0
...
// note: numberZ is a copy of the original. Its value is changed
	...
	if (i < numLen) {
		/* set any extra limbs to zero */
		memset(number + i, 0, (numLen - i) * sizeof(number[0]));
	}
	if (neg) {
